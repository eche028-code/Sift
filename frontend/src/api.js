...
export const api = {
  ...
  saveFragments: (ids) =>
    req("POST", `/api/notes/fragments/save${ids && ids.length ? `?ids=${ids.join(",")}` : ""}`),
  getTaxonomy: () => req("GET", "/api/codex/taxonomy"),
  putTaxonomy: (yaml_text) => req("PUT", "/api/codex/taxonomy", { yaml_text }),
  getExportDir: () => req("GET", "/api/codex/export-dir"),
  checkExportDir: (path) => req("POST", "/api/codex/export-dir/check", { path }),
